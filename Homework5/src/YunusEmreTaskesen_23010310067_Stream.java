...
public class YunusEmreTaskesen_23010310067_Stream {
	public static void main(String[] args) throws IOException {
		...
		System.out.println(toSortedForm("abcdcadabadbbdd"));
		System.out.println();
	...
	public static void multOf10() {
		int result = IntStream.of(55, 20, 19, 31, 40, -2, 62, 30).filter(n -> n % 10 == 0).max().getAsInt();
		System.out.println(result);
	}

	public static int sumAbsVals(int[] array) {
		return Arrays.stream(array).map(n -> Math.abs(n)).sum();
	}

	public static int countNegative(int[] array) {
		return (int) Arrays.stream(array).filter(n -> n < 0).count();//map(n -> 1) //.sum();
	...
	public static String pigLatin(String metin) {
		return Arrays.stream(metin.split(" ")).map(s -> s.substring(1) + "-" + s.charAt(0) + "ay").reduce("",
				(s1, s2) -> s1 + " " + s2);
	}
	public static String toSortedForm(String str) {
		return str.chars().sorted().mapToObj(c->String.valueOf((char) c))
				.reduce("",(n1,n2)->n1+n2);
	}
	public static int longestLine(String filename) throws IOException {
		int result= Files.lines(Paths.get(filename)).mapToInt(line->line.length())
				.max().getAsInt();
		return result;
	}
}
